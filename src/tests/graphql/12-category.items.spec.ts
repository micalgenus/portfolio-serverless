...
describe('Category items', () => {
  ...
  before(async () => {
    ...
    const result = await requestAsync({ query, variables: { id: 'user' } });
    category = result.body.data.getUserInfo.categories[result.body.data.getUserInfo.categories.length - 1]._id;
  });

  it('Success', async () => {
    const res = await requestAsync({ query, variables: { id: 'user', categoryFilter: [category] } });
  ...
  it('Filtering categories', async () => {
    ...
    const res = await requestAsync({ query, variables: { id: 'user', categoryFilter: [category], itemFilter: [item1, item2] } });
    expect(res).to.have.status(200);

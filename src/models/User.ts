...
import {
  ...
  decodeRememberMeToken,
  compareUserPasswordAndLogin,
...
class UserDatabase extends DataStore<UserTable> {
  ...
  async checkValidUpdateEmail(oldEmail: string, newEmail: string): Promise<boolean> {
    return this.checkValidUpdateUniqueItem('email', oldEmail, newEmail, checkValidEmail);
  }

  async checkValidUpdateId(oldId: string, newId: string): Promise<boolean> {
    return this.checkValidUpdateUniqueItem('id', oldId, newId, checkValidId);
  }
  ...
  async loginOAuthByPk(type: OAuth, id: string): Promise<string> {
    ...
    return createJwtToken(user, type);
  }
  ...
  async loginById({ id, password }): Promise<string> {
    ...
    return compareUserPasswordAndLogin(existId[0], password);
  }
  ...
  async loginByEmail({ email, password }): Promise<string> {
    ...
    return compareUserPasswordAndLogin(existEmail[0], password);
  }
